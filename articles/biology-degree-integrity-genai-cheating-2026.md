...
- **Instructors.** Compute your own course's exposure before redesigning anything: multiply each graded category's point weight by how vulnerable instructors rate that category — the procedure that produced 33% of points highly vulnerable and 81% at least somewhat vulnerable in these 38 syllabi.
- **Instructors.** Stop treating [[remote-proctoring|proctoring]] as the fix for take-home exams: proctored and unproctored outside-of-class exams were rated equally vulnerable (median 3), while only in-person proctored exams came in at median 0.
- **Assessment designers.** Rebalance points away from vulnerable low-stakes work rather than abolishing it, and name the trade-off to students and colleagues: removing the incentive also removes the reason that honestly engaged students did the work.
- **Administrators.** Treat [[assessment-validity|grade validity]] as a departmental responsibility — audit every offering's syllabus, and staff in-class accountability with circulating teaching or learning assistants where course size (61% of these offerings enrolled 200 or more) makes individual instructor monitoring impossible.
- **Researchers.** Replicate the syllabus-audit-plus-instructor-perception method in other departments, disciplines and terms, because a single department in a single term can only predict, not demonstrate, how widespread the exposure is.

## Limitations

- **One department, one institution, one term.** The paper states the central limit itself: as a case study it cannot speak to generalizability, and a prediction rather than a finding is offered that the problem is widespread.
- **Perceptions, not observed dishonesty.** The vulnerability figures rest on instructor estimates of exposure, so they measure the size of the opportunity rather than its use; the study cannot say how many students act on it, and the authors are explicit that it makes no such claim.
- **Lecture components only.** The syllabus analysis excludes lab work that constituted 15-40% of the grade in some courses, because syllabi did not disclose the assignment breakdown, so the percentages describe the graded portion that could be coded rather than the whole course.
- **Weighted toward the largest offerings.** The sample skews to large courses (61% enrolling 200+) and online sections (45%), which is also where vulnerability concentrates, so the aggregate sits closer to the exposure of the largest offerings than to a typical small seminar.
