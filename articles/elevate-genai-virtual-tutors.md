...
- **Designers.** Profile the whole pipeline on the least capable node before promising interactivity: TTS synthesis, not the 3B LLM, dominated end-to-end latency because it ran on the older 12 GB GPU, and streaming masked later generation so perceived latency dropped to near zero once speech began after an average 5-6 s time-to-first-audio. Do not treat the first generation step as the only cost to optimize.
- **Designers.** Make the teacher governance layer the control surface: expose system prompt, knowledge base, refusal and grounding policy, persona, voice, supportive-versus-neutral stance, scaffolding degree and pacing in a lightweight editor, so pedagogical framing stays persistent instead of depending on how a student prompts.
- **Designers.** Plan for the operational and accessibility load local-first deployment creates: schools take on updates, safety configuration, device management and policy alignment, and every configuration should keep captions, alternative inputs, reading-level controls, adjustable pace and switchable text-only or voice-only modes available. Default to aggregate and minimal logging, restricting raw transcript or audio retention to clearly justified cases, and measure the workload of configuration cycles and refusal follow-ups rather than assuming they will be sustained.
- **Instructors.** Frame an embodied spoken tutor as a pedagogical instrument inside lesson plans (warm-up, guided practice, revision stations) rather than a substitute for instruction, and configure it for coaching that elicits reasoning and partial attempts instead of delivering solutions. Because body and voice amplify authority, have the avatar signal uncertainty and invite verification against notes or textbooks, and prefer explicit classroom modes chosen by the teacher over silently inferred ones.
- **Researchers.** Evaluate these systems as sociotechnical interventions with controlled classroom studies, not with system benchmarks: whether always-available embodied tutors reduce productive struggle, shift help-seeking away from teachers and peers, and change ownership of learning outcomes is unresolved, while hallucination, over-reliance and deskilling, uneven benefits across student groups, and accountability drifting into surveillance through excessive logging remain structurally open risks. If on-demand tutoring is available, shift assessment toward process — oral defenses, in-class problem solving, stepwise derivations, reflective journaling — and assess students on documenting how assistance contributed to their work.

## Limitations

- No controlled user study was run: the evaluation pushed 20 representative student-style prompts through the end-to-end pipeline and measured only server-side generation and synthesis times, token counts, latency and perceived latency, with client-side latency unmeasured because it was negligible in practice. The authors state that evaluating learning outcomes and socio-technical effects in real school settings remains future work.
- Everything rests on one prototype and one domain: a single "History of Computer Science" use case with a one-time QLoRA fine-tune of Hermes-3B in Q8 and Coqui TTS, chosen after hardware screening showed larger models saturating 8-16 GB VRAM, so latency and answer quality are not established for other subjects or knowledge bases.
