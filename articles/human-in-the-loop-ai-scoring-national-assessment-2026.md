...
Under realistic assumptions the process would reduce by at least 50% the written productions requiring full human scoring, with the exact figure varying by edition because it depends on how many candidates fail on the other sections. The authors frame the guiding principle plainly: no outcome-affecting final decision is made without appropriate human oversight, and the whole framework exists to keep expert judgment in the loop at the decision points where it matters for [[assessment]] and [[summative-assessment|summative]] certification.
