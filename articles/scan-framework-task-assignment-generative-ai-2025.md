...
1. **SCAN adds a fourth zone to the ZPD.** Alongside "known to learner", "unknown to learner" and the ZPD itself, the framework adds "known to GenAI" — general, trained knowledge that lets a model complete a task with a generic rather than a specific approach. Visualized as a Venn diagram, the added circle carves Vygotsky's three zones into four sub-zones, labeled S, C, A and N, and is the paper's foundational move.
