...
7. **Upskilling is modelled as task migration.** With tasks counted as S, C, A and N, the authors define KL values (S over the total, A over the total, C over the total) and set upskilling as (A at t+1 − S at t+1) > 0 followed by (C at t+1 − A at t+1) > 0, i.e. a deliberate drift from Substitute to Aid to Complement through spaced practice, error tagging and explain-back prompts. Deskilling runs the other way, Complement to Aid, as the learner's role shifts from production to evaluation, and the authors expect such tasks to oscillate between Aid and Complement.
