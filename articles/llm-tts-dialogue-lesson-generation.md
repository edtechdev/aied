...
- **Automation should augment, not replace, educators.** The value of the pipeline lies in cutting production burden while an educator retains fact-checking, flow design, and final quality judgment — a design pattern for [[human-in-the-loop-ai]] in instructional production, not an argument for removing teachers.
...
- **Script "listenability" is a real engineering requirement.** Human-written narration tends to use written-language structures that TTS renders awkwardly; prompt-level optimization of the script is what makes the dialogue design viable at all.
