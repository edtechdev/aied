...
1. PAUSE is organized around four domains — D1 reasoning and critical thinking, D2 creativity and originality, D3 research and learning, D4 social and communicative capacity — each reported separately, with no overall composite because a single number across four domains of unequal weight would compress more than it reveals.
...
6. Two behavioral probes add texture beyond self-report: a claim-evaluation multiple-choice item drawn from a stratified pool over 10 reasoning-fallacy types, and a 90-second Alternative Uses Task on one of 12 common objects spanning 6 categories, scored for fluency and category-level flexibility.
...
PAUSE is positioned against validated instruments that occupy the neighboring measurement corners: the AI Dependence Scale (AIDep-22) of Wu et al. (2026), whose four factors include a cognitive-dependence dimension built on the offloading framework; the SAID questionnaire of Garcia Castro et al. (2025) for secondary-school students; the performance-based GLAT of Jin et al. (2024) and older [[ai-literacy|AI literacy]] scales; and the Offloading Score of Padmakumar et al. (2026), which estimates the fraction of cognitive effort offloaded by reconstructing a counterfactual workflow from behavioral logs (validated with n = 40 developers on programming tasks). PAUSE instead asks a descriptive questionnaire organized by four everyday domains and stores nothing — an explicitly modest answer to the call of Riley et al. (2025) for grounded evaluation frameworks that balance benefits against human-centric risks.
