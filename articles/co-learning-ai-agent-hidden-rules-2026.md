...
The GOHR asks a learner to sort coloured shapes into one of four buckets placed at the corners of the grid. Every placement is accepted or rejected with a happy or sad face, and the learner's job is to infer the hidden rule that governs acceptance. Rules can be trivial (colour match, nearest quadrant), moderately hard (shape match plus reading order) or composite (colour match combined with clockwise fill order), so difficulty becomes an experimental variable rather than a nuisance. The platform's crucial property is that the identical task can be posed to a human learner or to a machine, which makes human and AI learning directly comparable rather than merely analogous, and it has already been used that way in benchmarks and in reinforcement-learning comparisons.
...
- [[help-seeking]] — the accept-or-ignore choice that constitutes the study's behavioural signal
