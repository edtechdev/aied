...
Dierickx, Bjerknes, Opdahl, & Lindén (2026) address a gap in journalism education: prompting is increasingly recognized as an educational tool but remains underdeveloped there, with existing work focusing on AI use in newsrooms rather than translating journalistic work into structured [[pedagogy|pedagogical]] tasks. They ask: *How can prompting be structured as a pedagogical practice in journalism education, drawing on task definitions from natural language processing and [[reinforcement-learning|machine learning]]?*

## The taxonomy: four stages of the news workflow

The paper organizes journalistic work into four stages that mirror the news production process, each with tasks that can be meaningfully supported by LLMs while remaining compatible with journalism's norms of verification, editorial judgment, and ethical responsibility:

| Stage | Tasks |
|-------|-------|
| **Newsgathering** (collecting/structuring information) | Classification/clustering, detection/discovery, extraction, linking, summarization |
| **Sensemaking** (analyzing/interpreting to guide editorial decisions) | Comparison, evaluation, exploration, prediction, prioritization |
| **Editing** (revising/enriching/correcting content) | Augmentation, correction, generation, segmentation, simplification |
| **Publication/distribution** (adapting/disseminating content) | Adaptation, conversion/translation, generation, personalization, reformulation |
...
## Prompting as situated professional judgment

A central contribution is reframing prompting. Rather than treating it as a neutral technical act, the paper frames **task definition as a form of journalistic judgment** that must be learned, articulated, and critically assessed. Three ideas anchor this:
...
In the classroom, the approach shifts assessment from evaluating the "quality" of AI-generated text to articulating tasks, justifying prompt choices, and reflexively evaluating AI behavior. Rather than positioning human review as a final corrective step after an autonomous AI process, each task is treated as an ongoing negotiation in which journalistic judgment, professional norms, and algorithmic output are shaped together through iterative prompting and revision.
