...
- **Student-simulator tools can scale role play for [[online-teaching-and-learning|online teaching]] prep.** For large-scale online programs (e.g., Code in Place, Schoolhouse), simulated sections provide 24/7, low-cost practice that frees scarce human coaches for higher-value synchronous feedback.
- **Ground [[simulating-students|simulated students]] in real course content and knowledge states.** Contextualized memories tied to actual lecture materials (with per-document engagement levels) are what make the simulation pedagogically relevant, unlike generic [[conversational-ai|chatbots]] that hallucinate or lack domain knowledge.
