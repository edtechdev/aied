...
**Methodologically, this is a template.** Fitting standard models to logged interaction data and then simulating at scale lets designers pre-test sequencing and constraint policies across more than one domain before any student meets them — a bridge between [[learning-analytics|learning analytics]] and system redesign that costs a simulation run rather than a semester. The [[simulating-students|simulated learners]] here are statistical rather than [[llm]]-based agents, which is worth noting as the two strands of learner simulation develop in parallel: this one buys interpretability and parameter grounding, where generative agents buy behavioral richness.

**The autonomy literature still stands, but its benefits are uneven.** Because strategy diversity is real and consequential, constraint design has to be built around the distribution of learner behavior rather than an average learner — and the paper's own caveats point the same way.
