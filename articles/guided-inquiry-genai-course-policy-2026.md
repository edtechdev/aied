---
title: "A Guided Inquiry Approach to Students Co-Designing Generative AI Course Policies"
created: "2026-08-31T09:06:11-04:00"
updated: "2026-09-16T15:58:20-04:00"
type: article
tags: [generative-ai, ai-education, governance, agency, higher-ed, ai-literacy, teacher-role, student-engagement, ethics, curriculum-design]
connected_faqs: [course-ai-policy]
sources: ['raw/papers/2608.28501.md']
confidence: high
research_method: [qualitative, thematic analysis]
audience: [instructors, administrators, curriculum designers]
level: [higher ed]
category: [policy, curriculum design, engagement]
---

> **Synthesis:** Hingle and Johri (2026) report an exploratory study in which students co-designed a [[generative-ai]] course [[educational-policy-ai|policy]] through a guided inquiry activity. Students first drafted individual proposals for appropriate and [[ethics|ethical]] GenAI use, then refined them collaboratively by incorporating diverse stakeholder perspectives. Analysis of student artifacts and group discussions showed participants prioritized training for students and instructors, standardized [[ai-use-disclosure|disclosure]] procedures, stronger institutional support, and greater involvement in GenAI-related decision-making. The study offers [[pedagogy|pedagogical]] insights into how policy co-design can surface student values and sensemaking about governing AI use in educational contexts.

## Co-Designing GenAI Policy Through Guided Inquiry

Institutions and instructors increasingly confront questions about how to support learning while addressing the ethical and institutional concerns raised by [[generative-ai]]. Yet students — the people who make most day-to-day decisions about AI use — are largely absent from the [[educational-policy-ai|policy-making]] process. This exploratory study examines a guided inquiry activity in which students took an active role in designing a GenAI course policy.

The activity progressed through several stages: students first completed readings on AI use, then used GenAI to brainstorm initial policy ideas; next, they articulated their own perspectives in a written assignment and an individually designed course policy; finally, they collaborated with peers to incorporate diverse stakeholder perspectives into a collective policy. Two [[research-methods-aied|research]] questions guided the study: what practical factors students prioritize in their GenAI use policies and how they justify these choices, and how participants reflect on the policy design process.

## What Students Prioritize

1. **Training for students and [[teacher-role|instructors]].** Participants emphasized the need for clear guidance and skill development around appropriate GenAI use — a demand that maps to [[ai-literacy]] and [[teacher-education]].
2. **Standardized [[ai-use-disclosure|disclosure]] procedures.** Students wanted consistent, transparent expectations for when and how to disclose AI use in academic work.
3. **Stronger institutional support.** Rather than relying on individual instructors, students called for coherent, institution-level backing for [[educational-policy-ai|GenAI policy]].
4. **Greater involvement in decision-making.** Participants wanted a meaningful role in shaping the rules that govern their learning, reflecting a desire for [[agency]] in AI governance.

Students described the policy design process itself as valuable, noting that it forced them to engage with multiple perspectives and the inherent trade-offs involved in governing AI use. This positions policy co-design as both an outcome and a learning activity: it surfaces student values and concerns while developing their capacity to reason about responsible AI use.

## Implications

The findings support treating students as partners in [[governance|AI governance]] rather than passive subjects of policy. A guided inquiry approach can serve as a practical pedagogy for eliciting student perspectives on [[generative-ai]] use, and it complements broader institutional efforts to develop [[ai-literacy]] and responsible-use norms across [[higher-ed]]. For instructors, the activity offers a low-cost way to translate student voice into course-level policy while building students' capacity for ethical reasoning about AI.

## Connected Concepts
- [[generative-ai]]
- [[ai-education]]
- [[governance]]
- [[educational-policy-ai]]
- [[agency]]
- [[ai-literacy]]
- [[ai-use-disclosure]]
- [[academic-integrity]]
- [[teacher-role]]
- [[student-engagement]]
- [[higher-ed]]
- [[ethics]]
- [[curriculum-design]]

## Connected Articles
- [[ai-adaptation-gap-higher-education-2026]] — The AI Adaptation Gap in Higher Education
- [[chang-should-i-tell-my-teacher-ai-disclosure-2026]] — "Should I Tell My Teacher?" Student AI Disclosure Practices
- [[kirsanov-beyond-detection-ai-online-assessments-2026]] — Beyond Detection: How Students Use—and Hide—AI
- [[reclaiming-epistemic-agency-co-agency-2026]] — Reclaiming Epistemic Agency
- [[student-perceptions-ai-study-productivity-2026]] — Students' Perceptions of Artificial Intelligence Tools for Study Productivity and Learning: An Exploratory Survey Study

## Citation
Hingle, A., & Johri, A. (2026). [*A Guided Inquiry Approach to Students Co-Designing Generative AI Course Policies*](https://arxiv.org/abs/2608.28501). arXiv:2608.28501.
