...
7. **The techniques are near-free in compute.** Weighted scoring and confidence extraction reuse log probabilities already computed in the forward pass but normally discarded, and a randomized API [[benchmark]] found logprob-returning calls between 1.7% and 5.7% faster, i.e. indistinguishable from latency noise. Only ensembling multiplies inference cost, roughly doubling API calls for two models, though calls can be issued in parallel.
...
- **Assessment professionals.** Route the least confident one in five responses to human review rather than re-scoring everything. In that scenario fine-tuned GPT-3.5 moved from r = 0.781 to r = 0.822 (RMSE 0.5990 to 0.5544) while manual scoring effort fell by roughly 80%.
- **Assessment designers.** Extract the model's token-level confidence on every score and treat it as a per-response uncertainty figure: absolute error falls as confidence rises (β = -0.602, p < .001), which makes confidence a routing signal rather than a global verdict on the tool.
- **Assessment designers.** Replace the single best-guess score with a probability-weighted mean over the top candidate scores. Fine-tuned GPT-3.5 rose from r = 0.781 to r = 0.816 and RMSE fell from 0.5990 to 0.5235, and because the authors varied the number of contributors from 1 to 5, the top five capture the available gain.
- **Assessment designers.** Stack the two techniques (r = 0.846, RMSE = 0.4848) and add a second model only when it is architecturally different: pairing two fine-tuned models reached r = 0.823 against r = 0.816 for the best single model, while a third added nothing.
- **Assessment professionals.** Do not treat confidence-based exclusion as a fairness-neutral filter. Confidence partly tracks human rater disagreement (r = -0.379, p < .001), so exclusion also removes genuinely ambiguous responses, and the task's own rater-to-consensus ceiling of r = 0.88 limits what a reliability gain can mean for high-stakes use.
