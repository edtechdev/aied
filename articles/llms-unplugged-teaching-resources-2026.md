...
7. The grid variant (paper grids plus dice) connects to probability concepts in the [[math-education|mathematics]] curriculum, while the bucket variant (cut-up words organised into buckets) supports younger learners or settings where dice maths would be a distraction; both teach identical concepts.
...
9. The synthetic-data extension demonstrates collapse behaviour directly: students train a new model on their own generated text and watch patterns degrade as rare words vanish and common phrases dominate.
