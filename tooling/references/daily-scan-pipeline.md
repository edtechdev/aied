...
- **execute_code `read_file` incompatibility**: In `execute_code` scripts, do NOT use `read_file` from `agent_tools` — it returns a dict with unpredictable key structure in the sandbox. Use Python's built-in `open()` and `os.listdir()` for all file reading. `write_file` from agent_tools is safe to use in execute_code for writes.
