...
```
Run from `<AGENT>/research-wiki/` (the skill directory). If you get `No such file or directory`, the workdir is wrong — search for the script with `search_files(pattern='generate-static-site.py', target='files', path='<AGENT>')` to resolve the correct path. Do NOT use `wiki-static-export` as the workdir — that directory does not exist.
