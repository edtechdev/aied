...
- Newest `content/en/concepts/daily-digest-YYYY-MM-DD.md` filename = **authoritative** last scan date (here `2026-07-15`). `memory` tool was unavailable in cron — persist the date in `log.md` + the digest filename instead.
...
- **Script B (`execute_code`):** back-links to 5+ existing pages (regex `## Related Pages` insertion; verify with `os.path.exists` + dedup check); full index rebuild from ALL `content/en/concepts/*.md` (every page type, sort by filename lower); `journal.md` regen (strict 3-line entries, em-dash `—` separator, grouped by `created`).
...
- **Three-way reconcile**: `len(content/en/concepts/*.md)` == `# of - [[ lines in index.md` == `# of static-site/pages/*.html`. (This run: 459/459/459.)
