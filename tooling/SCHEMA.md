...
**Readership is not subject matter, and the labels say so.** `audience` (rendered **Intended audience**) is who the page is written for - a role vocabulary of instructors, learners, researchers, curriculum designers, software developers - and holds no concept slugs. The subject-matter fields, `institutions` (**Institutions and policy**) among them, carry the concept slugs of their registry sections that the page analyses, such as AI governance and regulation.

**The `stakeholders` field (People) was retired 2026-09-21.** It restated `audience` on the pages that used both (174 pages read `audience: [learners]` beside `stakeholders: [student-experience]`), and its two most-used values were not people at all: `student-experience` is a learner-experience concept and `teacher-ai-competency` a capability, so the field had no definition that could be applied consistently. Readership is now the single question it answers, and `audience` answers it. The People section keeps its concept pages for browsing (like AI in the disciplines, it backs no field); `teacher-ai-competency` moved to Foundations of AI in education and `student-experience` to Learning and instruction, where they are usable as `foundations` and `pedagogy` values.
...
**Every metadata field label links to its umbrella concept page.** Where a field's vocabulary has one page that reads as the umbrella for the whole field, the metadata table at the foot of a page links the field NAME there - *Study design* and *Research methods* both to the research methods page, *Discipline* to discipline-specific AI in education, *Technologies and techniques* to the AI technologies page. The map is generated (`FIELD_UMBRELLAS` in `src/data/metadataLinks.ts`) and the link is dropped if the page does not exist in the build, so a renamed or missing umbrella degrades to plain text rather than a dead link.
...
audience: administrators | assessment designers | assessment professionals | curriculum designers | educational technology developers | faculty developers | institutions | instructional designers | instructors | learners | learning analytics designers | librarians | medical educators | parents and families | policymakers | researchers | software developers
...
- YAML parses an *unquoted* ISO timestamp into a JS `Date` in UTC, shifting an Eastern-evening value to the next calendar day. **Always quote** the value so the schema preserves the original string. `tooling/scripts/check-frontmatter-dates.py` (gate 11) enforces this: it fails on any date field that YAML has auto-typed, and on `created`/`updated` that are not full quoted timestamps with an offset. The damage it guards against is a vault editor (Obsidian and similar) re-serialising a frontmatter block and dropping the quotes — harmless-looking, but it moves the page's calendar day for the journal, sidebar and RSS.
