...
1. **Save raw source** → `raw/papers/[arxiv_id_or_slug].md` with frontmatter (source_url, ingested_date, sha256)
   ...
   - **Record the study's tool generation.** While the full text is in hand, note the model/version it used and its data-collection window (method section: `GPT-3`, `GPT-4`, "free version", collection dates) in the raw frontmatter or your notes, and report both in the ingest summary. A comparison run on a superseded generation (pre-2024 / GPT-3.5-era) is a screening flag for the maintainer: a study can be well designed and still describe a tool that no longer exists, and the mechanical gates will not catch it. See the ingestion skill's item 11 for the dispositions.
...
2. **Create article page** → `content/en/articles/[slug].md` (this is the wiki's page type for individual papers)
   - Frontmatter: title, created, updated, published, type: article, sources, confidence
   - **Populate the optional structured metadata facets when determinable from the paper:** `research_method` (survey, systematic review, experiment, case study, benchmark, etc.), `discipline` (math education, physics education, cs education, etc.), `audience` (instructors, learners, researchers, administrators, software developers), `level` (early childhood, k 12, higher ed, adult learning), `page_kind` (framework, synthesis, evaluation). See `tooling/SCHEMA.md` for the closed vocabularies. **There is no `tags` field: every concept a page touches is named in a typed field.** Fill the seven facet fields (`foundations`, `pedagogy`, `technology`, `assessment`, `methods`, `institutions`, `ethics`) with concept slugs taken from the matching section of `concepts.registry.yaml` — a value of the wrong kind fails the build. **After writing pages, run `python3 tooling/scripts/validate-facets.py`.** The typed facet fields
(`foundations`, `pedagogy`, `technology`, `assessment`, `methods`, `institutions`, `ethics`) each take concept slugs
filed under the matching section of `concepts.registry.yaml`, and a value of the wrong kind — a technology slug in
...
**Fill `research_method`, `audience` and `level` for every new article unless the page genuinely gives no basis; the single most common defect on ingested pages is a paper whose design is obvious from the body and absent from the frontmatter.** When a paper's method or discipline does not fit the closed vocabularies, extend the list in `src/content.config.ts` deliberately rather than writing a value outside it, then run `python3 tooling/scripts/audit-metadata.py --write-docs` so `tooling/SCHEMA.md` stays in step. Before finishing the run, verify with `python3 tooling/scripts/audit-metadata.py --missing research_method` and `--strict`.
   - **`created` MUST be today's date (ingestion date), NOT the paper's publication date.** The right-sidebar "Recent Articles" and the Journal page sort by `created`, so setting it to the paper's pub date buries new articles under older ones and they never appear as "new." Use `created: [TODAY]` (same as `updated`). **Set `published` to the paper's own publication date** (arXiv submission day from the `arXiv:NNNN.NNNNNv1 [cat] DD Mon YYYY` line, the journal's publication date, or the year when that is all the source gives): `YYYY-MM-DD`, `YYYY-MM` or `YYYY`, never an invented day, and omit the field entirely if the source does not state one. It renders in the page header as `published … · added …` and feeds the JSON-LD `ScholarlyArticle` date, so a paper published last year no longer reads as if it were published the day it was ingested.
   - Body structure, in this FIXED order: synthesis blockquote → `## Key Findings` (contiguous numbered list, 5-7 items, ~25-35 words each) → 3-4 `##` prose sections (~120-180 words each) → `## What this means for practice` → `## Limitations` → `## Connected Concepts` → `## Connected Articles` → `## Citation` (APA, hyperlinked title; LAST section, per the standing hard rule). Nothing goes after `## Citation`; the template appends Connected FAQs, the metadata table and source buttons.
     ...
     - **Length budget: 750-1,000 words** for the body (frontmatter end → `## Connected Concepts`). The section auditor prints the count and lists over-budget pages — `python3 tooling/scripts/audit-article-sections.py --slugs-file <slugs>`; it reports rather than fails, so read the output. Trim to the budget before committing; never bolt sections on top of an already-full body. The two sections above are paid for out of the existing body, never bolted on top.
...
3. **Update/create concept pages** → `content/en/concepts/[slug].md` for topics that synthesize multiple papers
