...
2. **Create article page** → `articles/[slug].md` (this is the wiki's page type for individual papers)
   ...
   - **Populate the optional structured metadata facets when determinable from the paper:** `research_method` (survey, systematic review, experiment, case study, benchmark, etc.), `discipline` (math education, physics education, cs education, etc.), `audience` (instructors, learners, researchers, administrators, software developers), `level` (early childhood, k 12, higher ed, adult learning), `page_kind` (framework, synthesis, evaluation). See `tooling/SCHEMA.md` for the closed vocabularies. `tags` must be real concept slugs (never invent new tags). These power the search page filter facets. **After writing pages, run `python3 tooling/scripts/derive-facets.py --apply`.** The typed facet fields
(`foundations`, `pedagogy`, `technology`, `assessment`, `stakeholders`, `ethics`) are DERIVED from each page's
concept tags by looking up which section of `concepts.registry.yaml` the concept belongs to, so this is mechanical
and must never be guessed by hand. `npm run verify` fails if a page's facets are out of sync with its tags.
