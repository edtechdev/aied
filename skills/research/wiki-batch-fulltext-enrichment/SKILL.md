...
- **Subagent timeout mid-write** corrupts frontmatter (missing closing `---`, cut-off fields). Repair from `git show HEAD:articles/<slug>.md` frontmatter + preserve the enriched body, then re-verify. A wave whose children all time out can still have written every page: check the files (frontmatter opens and closes, `## Citation` last, body words in range) before re-dispatching anything.
