...
Design reference header (`https://docs.example.com/configuration/theming`): a single `flex nowrap` row (h-16), nav collapses on mobile, and search stays as a compact inline item (icon+trigger) instead of dropping to its own row — the pattern the maintainer points to for "one row on mobile."
