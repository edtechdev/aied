...
The repo's `tooling/` folder is the **distributable copy** of the wiki's maintenance pipeline — files that let a stranger clone the repo and run their own version of this research wiki. It mirrors the active `research-wiki` skill under `~/.hermes/skills/research-wiki/`:

| Repo `tooling/` | Active skill (`~/.hermes/skills/research-wiki/`) |
|---|---|
...
The other user-owned wiki skills live under the repo's **`skills/`** directory (not `tooling/`), mirroring `~/.hermes/skills/`:
| Repo `skills/research/` | Active skill (`~/.hermes/skills/`) |
|---|---|
...
Do NOT assume the active `~/.hermes` skill is the newer/correct side and `cp` its files over the repo. **The repo `tooling/` copy was, in places, MORE current than the active skill:**
...
3. Propagate the GOOD repo version INTO the active skill: `cp tooling/scripts/<file> ~/.hermes/skills/research-wiki/scripts/<file>`.
...
Since the jobs load from the ACTIVE skill's `cron/`, editing only the repo copy has no runtime effect — sync to `~/.hermes/skills/research-wiki/cron/` too.

## Full verification checklist (end of any tooling-sync pass)

```bash
cd <WIKI>
ACT=~/.hermes/skills/research-wiki
...
# user-owned wiki skills under skills/ mirror ~/.hermes/skills/
diff    skills/research/wiki-epub-export/SKILL.md  ~/.hermes/skills/research/wiki-epub-export/SKILL.md && echo "epub SKILL: IDENTICAL"
diff    skills/research/wiki-inline-links/SKILL.md ~/.hermes/skills/research/wiki-inline-links/SKILL.md && echo "inline-links SKILL: IDENTICAL"
diff    skills/research/wiki-inline-links/scripts/inline_link_scan.py ~/.hermes/skills/research/wiki-inline-links/scripts/inline_link_scan.py && echo "inline_link_scan: IDENTICAL"
