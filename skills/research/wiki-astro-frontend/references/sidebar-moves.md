...
4. **Do NOT auto-regenerate the EPUB/PDF** after a sidebar move or label change (Doug 2026-09-08): epub/pdf are local committed artifacts rebuilt ONLY on explicit Doug request. `build-epub.py` parses `conceptIndex.ts` for chapter/group structure, so if Doug does direct an epub/pdf rebuild AFTER a move/label change, regenerate then — but do not run it proactively; the drift risk is accepted until Doug asks. (Still run step 3 for llms files, which are cheap and always kept in sync.)
