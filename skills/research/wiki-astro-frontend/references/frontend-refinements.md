...
- **Style** (design-reference-inspired): two bordered cards `.pn-prev`/`.pn-next` in a `1fr 1fr` grid (stacking to `1fr` below 640px), each with an uppercase muted caption (`← Previous` / `Next →`), the adjacent page's title, `hover → primary border/tint`. Separated from content by a `border-top` + `margin-top:3rem`. First page shows only Next; last FAQ only Previous.
