...
1. **Read the article page** (`content/en/articles/<slug>.md`) to find defects and gauge thickness. Also check the raw source path in its `sources:` frontmatter.
...
- Detect: for each modified `content/en/articles/*.md`, check the first ~40 lines for a closing `---` and for each required field (`title:`, `created:`, `updated:`, `type:`, `sources:`, `confidence:`). Restore the page's typed fields too (`pedagogy`, `technology`, `assessment`, `methods`, `institutions`, `ethics`, `foundations`, plus `research_method`, `discipline`, `level`, `audience`, `page_kind`). There is no `tags:` line any more: the field is retired and the schema no longer accepts it (see Pitfall #11).
...
```python
import re
raw = open('content/en/articles/<slug>.md', encoding='utf-8').read()
...
Every `content/en/articles/<slug>.md` body follows this order:
...
4. Fix the registry, fix every page already mis-linked (`grep -rn '\[\[<wrong-slug>|<phrase>\]\]' content/en/articles/ content/en/concepts/`), then `python3 tooling/scripts/gen-concept-artifacts.py` and `python3 tooling/scripts/check_concepts.py`.
...
- **Facet fields**, one per registry section, whose values must be concept slugs filed under THAT section: `pedagogy` (Learning and instruction), `technology` (Technologies and techniques), `assessment` (Assessment and measurement), `methods` (Research methods and evaluation), `institutions` (Institutions and policy), `ethics` (Equity, ethics, and responsible use), `foundations` (Foundations of AI in education). A value of the wrong kind (a technology slug in `pedagogy`) fails the build. The allowed values are generated from `concepts.registry.yaml` into `src/data/facetVocab.ts`.
