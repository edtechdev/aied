...
- **Content collection** `content/en/faqs/` registered in `src/content.config.ts` (glob `content/en/faqs/*.md`; schema: `title`, `created`, `updated`, `tags`). FAQ slugs live alongside `content/en/articles/` and `content/en/concepts/`.
