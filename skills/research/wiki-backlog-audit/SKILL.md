...
reconciling the backlog file against `articles/` and keeping the file's own counts honest.
...
2. **Index every `articles/*.md`**: harvest DOIs with `10\.\d{4,9}/[^\s"')]+` from the **whole page**,
...
6. **Confirm a hit is a real page**: the file exists in `articles/` with a real body, and its
...
live. `articles/*.md` is the source of truth; use `dist/` only as a weak second signal.
...
- `scripts/backlog_overlap_check.py` — parses the backlog, indexes `articles/`, prints per-entry
