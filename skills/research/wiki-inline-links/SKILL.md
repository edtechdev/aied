...
2. **Narrative-only ADDING.** The inline links you insert go ONLY in the body narrative (between the frontmatter and the `## Connected` sections). **NEVER add or modify links in the `## Connected Concepts` or `## Connected Articles` lists** — those lists are maintained by the back-link/enrichment process (see `research-wiki`), not by this pass. This pass reads the connected lists only to (a) avoid duplicating a link already present and (b) verify them for defects (below), never to edit them.
   - **NEVER touch the `## Citation` section** (2026-09-01 hard rule): an article's citation is APA text whose **title is a single hyperlink to the source** (`Author (Year). [Title](https://doi.org/…). *Journal*…`). The citation must contain **exactly one link** (the title→source) and **zero `[[wikilinks]]`**. An earlier bug corrupted 16 citation titles by turning words inside the title into `[[wikilink|word]]` (e.g. `AI [[problem-solving]]`, `[[k-12|secondary school]]`). The scanner's `get_narrative()` now stops at `## Citation` (whether it sits before or after `## Connected`) so citation text is never scanned or linked. If you ever edit a citation by hand, keep ONLY the title hyperlinked and the rest as plain prose/italics — never add inline wikilinks to the authors, title words, journal, or volume.
   - **The source link lives ONLY in `## Citation` — never a standalone body link (2026-09-01):** the narrative body (frontmatter → `## Connected`/`## Citation`) must contain **no** standalone source/PDF/DOI link line. Do NOT add a `📄 [PDF](https://…)`, `📄 arXiv · [PDF](…)`, `📄 [Full article](…)`, or `📄 DOI: …` line right after the frontmatter or anywhere in the body — that is redundant (the `## Citation` title→source link already carries the source) and the maintainer flags it. If an article's raw source or an open-access PDF is worth surfacing, it belongs only as the `## Citation` title link. `scripts/check_list_formatting.py` now flags any such stray source-link line (excluding the `## Citation` section), so the HARD GATE catches this class automatically.
...
**`REJECT_PAIRS` is the semantic reject list (2026-09-12).** Lexical matching cannot tell that "a public **research** university" is not `[[research-methods-aied]]`, that "a sense of **achievement**" is not `[[learning-gains]]`, or that "prompting was **bilingual**" is not `[[multilingual-learning]]`. `REJECT_PAIRS` holds `(matched phrase, target slug)` pairs that are **never** linked, in either mode, and each entry is an actual false positive found on a real page. Report mode prints them as `(suppressed by REJECT_PAIRS: ...)` so the suppression stays visible rather than silently dropping evidence. **Rule: whenever you hand-revert an auto-applied link as semantically wrong, add that `(phrase, slug)` pair to `REJECT_PAIRS`.**

**Reject the narrowest phrase that is actually wrong (2026-09-12).** Before rejecting a word outright, check whether it is ambiguous between two senses: if only one sense is wrong, reject that *phrase* and add qualified phrase aliases for the legitimate sense, so the word is still linkable where it is right. The worked example is `achievement`: the outcome sense (student achievement, academic achievement, learning achievement, prior achievement, achievement gaps) belongs to `[[learning-gains]]` and those phrases are aliases in it, while the *felt* sense ("**sense of achievement**", "feeling of achievement") is `[[motivation]]`/`[[self-efficacy]]` territory and is what `REJECT_PAIRS` blocks. Bare ambiguous `achievement` stays in `AUTO_APPLY_DENYLIST` — report-only, so a human judges it in context but nothing auto-links it. Achievement-goal theory ("achievement goals", "goal orientations") is a motivation construct, aliased to `[[motivation]]`. Other current entries cover `research`/`researchers`→`research-methods-aied`, `teaching`→`teacher-role` (verb sense), `evaluation`/`ai evaluation`→`ai-ed-evaluation`, `bilingual`→`multilingual-learning`, `transparency`→`explainable-ai` (institutional transparency, not XAI), `engagement`/`active engagement`→`student-engagement` (ambiguous bare usage), `situated`→`situated-learning` (adjectival), and `framing ai`→`framing-ai-use-for-students`.
...
0. **Frontmatter/body merge check (repo-wide, cheap):** parse every page with `re.match(r'^---\n(.*?)\n---\n', raw, re.DOTALL)`. Any page where that fails has its closing `---` fused to the first body line (`confidence: high\n---## Core Contribution`). Astro then treats the whole file as body: the page loses its title/date/tags and silently drops out of index/journal regeneration. Repair by inserting `\n\n` after the delimiter. Nine such pages were found this way (2026-09-14) only because an index rebuild dropped their entries — so always reconcile the parsed-page count against `ls <collection>/*.md | wc -l` before trusting a regenerated index or journal.
...
## Pitfalls
- **A `]` inside a wikilink LABEL defeats the standard regex (2026-09-17).** A label pattern of `[^\]]+` stops at the first `]`, so a legitimately bracketed label such as `[[paper-slug|"Young Scholar[s] on the Beat": multimodal composition]]` never matches and the raw `[[slug|label]]` is printed instead. It surfaces in three places at once: the visible page, `llms.txt`, and the offline EPUB/PDF. **Rule:** match the label non-greedily up to the closing `]]` — `\[\[([^\]|]+)(?:\|([\s\S]+?))?\]\]` — never `[^\]]+` for the label. The pattern now lives once in `tooling/scripts/wikilink_text.py` (shared by `build-epub.py` and `generate-llms-files.py`) and is mirrored in `src/lib/markdown.ts`; after touching any of them, rebuild and grep the outputs for a stray `[[`.
- **Loose blank lines inside the CONNECTED lists are NOT caught by the list checker, and they lift each item's type icon onto its own line (2026-09-21).** `check_list_formatting.py` flags ordered-list splits and stray source-link lines only, so a BULLET list whose items are separated by blank lines passes the gate as `Defects: 0` while the renderer treats the whole list as loose: every item gets wrapped in a `<p>`, and the page-type icon (an inline-block `<svg>` the pipeline injects as the link's first child) then renders ABOVE the title instead of beside it. Built HTML is the tell: loose reads `<li><svg ...></svg><p><a ...>Title</a></p></li>`, tight reads `<li><a ...><svg ...></svg>Title</a></li>`. Fix by collapsing ONLY the blank lines that sit between two bullet lines, and only inside `## Connected ...` sections (one pass removed 416 such lines across 202 pages; four pages needed a second pass because a section boundary confused the slice, but a `^- ` / blank / `^- ` search still matched). Verify from the BUILD output rather than the source: scan every built page's `.connected-list` block for `<p>` and require zero (3,380 lists checked).
- **A connected list far above the corpus norm is a maintainer complaint waiting to happen.** Median list length is about 15 concepts and 17 articles per concept page; beyond roughly the 90th percentile (25 / 47) they stop being wayfinding pages and become dumps. The AI Literacy page had reached 58 concepts and 75 articles when it was flagged. Trim to the load-bearing set (for a hub concept: the constructs that define it, the levels it applies at, plus the frameworks, scales and reviews that carry its evidence), keep the existing description text on the items you keep, and drop the rest rather than rewording them.
- **NEVER hand-write a wikilink regex by string-formatting literal brackets (2026-09-14 near-miss).** `pat = '[[%s|%s]]' % (re.escape(slug), re.escape(disp))` looks safe but the literal `[[`, `|` and `]]` are unescaped regex metacharacters — the pattern degrades into a character class that matches bare `]]`, and `re.sub(pat, disp, s)` then replaces EVERY `]]` in the file with the display text, silently destroying every link on the page. Six freshly written article pages were wrecked this way and had to be regenerated. **Rule:** escape the delimiters too (`r'\[\[%s\|%s\]\]'`) or, better, revert/undo links with plain `str.replace('[[slug|display]]', 'display')`, which needs no regex at all. After any scripted link edit, assert `raw.count('[[') == raw.count(']]')` for every file touched before moving on.
...
- **Caret citation markers (`^[[slug]]`) are deprecated (2026-08-30)**: pages previously used `^[[article-slug]]` footnote-style markers for in-narrative citations. On the site they render as a stray `^` plus link, and in the EPUB/PDF they render as footnotes. **The maintainer prefers minimizing footnote-style citations when authoring pages.** The repo-wide convention is now **parenthesized hyperlinks**: `^[[slug]]` → `([[slug]])` (and `^[[slug|label]]` → `([[slug|label]])`). When you see a caret-citation marker, wrap the whole `[[...]]` in parentheses and drop the `^`. Self-references (`^[[own-slug]]`) are removed entirely.
