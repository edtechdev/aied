...
**`REJECT_PAIRS` is the semantic reject list (2026-09-12).** Lexical matching cannot tell that "a public **research** university" is not `[[research-methods-aied]]`, that "a sense of **achievement**" is not `[[learning-gains]]`, or that "prompting was **bilingual**" is not `[[multilingual-learning]]`. `REJECT_PAIRS` holds `(matched phrase, target slug)` pairs that are **never** linked, in either mode, and each entry is an actual false positive found on a real page. Report mode prints them as `(suppressed by REJECT_PAIRS: ...)` so the suppression stays visible rather than silently dropping evidence. **Rule: whenever you hand-revert an auto-applied link as semantically wrong, add that `(phrase, slug)` pair to `REJECT_PAIRS`** so the same mistake cannot recur — a batch linking pass re-reads the whole dictionary, so an unfixed false positive returns on the next page that contains the word. Current entries cover `research`/`researchers`→`research-methods-aied`, `teaching`→`teacher-role` (verb sense), `evaluation`/`ai evaluation`→`ai-ed-evaluation`, `achievement`→`learning-gains`, `bilingual`→`multilingual-learning`, `transparency`→`explainable-ai` (institutional transparency, not XAI), `engagement`/`active engagement`→`student-engagement` (ambiguous bare usage), `situated`→`situated-learning` (adjectival), and `framing ai`→`framing-ai-use-for-students`.
