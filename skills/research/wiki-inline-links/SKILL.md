...
- **execute_code vs file tools**: use Python `open()`/`os` for all file I/O inside `execute_code` (not `agent_tools.read_file`, which returns an incompatible dict). Use `write_file`/`terminal` carefully — the run-the-script-from-/tmp pattern (`write_file` a script then `terminal('python3 /tmp/s.py', workdir='<WIKI>')`) is the most reliable in cron.
