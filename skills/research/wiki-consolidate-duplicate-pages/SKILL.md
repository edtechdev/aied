...
## When to use
- Two `content/en/articles/<a>.md` and `content/en/articles/<b>.md` pages reference the **same** source: identical `sources: ['raw/papers/<x>.md']` frontmatter, same DOI, or same source_url. This is the duplicate signal.
...
4. **Delete** the non-canonical `content/en/articles/<slug>.md`.
...
9. **Verify** (before build):
   ...
   - 0 broken links across all touched pages (every `[[target]]` resolves to `content/en/concepts/` ∪ `content/en/articles/` filenames ∪ `conceptRedirects.ts`).
