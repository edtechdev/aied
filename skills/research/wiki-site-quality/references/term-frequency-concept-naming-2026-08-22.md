...
over each directory separately — content/en/articles/, content/en/concepts/, raw/papers/ — because the three have
...
for t in "desirable difficult" "cognitive friction" "productive friction" "productive struggle" "cognitive offloading"; do
  echo "== $t =="
  echo "  articles: $(grep -ril "$t" content/en/articles/ | wc -l) files / $(grep -rio "$t" content/en/articles/ | wc -l) hits"
  echo "  concepts: $(grep -ril "$t" content/en/concepts/ | wc -l) files"
...
- **articles/ + content/en/concepts/** counts what the *wiki's own writing* calls it day-to-day. This
...
counting `grep -rli` across content/en/articles/ AND check no existing concept page already covers it
(`ls content/en/concepts/ | grep`). Report the file-count, the top articles, and which existing concept
...
- "prior knowledge" → 19 articles, covered inline in 7 concept pages → created
  `content/en/concepts/prior-knowledge.md`.
- "network analysis" family → ENA in 24 articles, TNA/SNA in 1 each → created umbrella
  `content/en/concepts/network-analysis.md` (ENA/TNA/SNA variants, distinct from the data-structure
  `knowledge-graph`).
