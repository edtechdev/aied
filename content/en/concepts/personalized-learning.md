...
updated: "2026-09-26T00:46:11-04:00"
type: concept
...
Where the planners above fail on the learner side, a complementary line of work asks what makes a path well formed in the first place. [[personalized-learning-path-ctml-2026|Mochocki et al. (2026)]] observe that personalized learning paths are typically matched to learners through learning styles, which lack empirical support, and derive their criteria instead from the [[theories-and-frameworks|Cognitive Theory of Multimedia Learning]] (CTML): a material rubric scoring each item against 12 CTML principles on a 1 to 4 scale, plus a path rubric adding nine selection categories and three sequencing categories. Their formal treatment splits path construction into material selection and sequencing, proves selection NP-complete by reduction from Subset Sum even when only the time interval objective is considered, and shows that many-to-many [[retrieval-spacing-interleaving|interleaving]] and prerequisite sequencing are likewise NP-complete while difficulty sequencing and the many-to-one variants are solvable in polynomial time. This is the sharpest account in the knowledge base of *why* path sequencing resists exact solution, and it shifts the design question from which theory of the learner to use toward which validated theory of the material. The evaluation itself ran on a 67-node computer science [[knowledge-graph]], profiles from 12 graduate students, and 365 rated materials, solved with NSGA-II for selection and a Random Hill Climber or simulated annealing for sequencing; returned paths averaged 3.19 (Student 11) to 3.54 (Student 9) on the four-point rubric, and the Random Hill Climber reached a mean of 4.00 on difficulty, prerequisite, and interleaving for most students, which the authors read as evidence that real instances are easier than the proofs suggest. Tension surfaced where goals and time budgets collided: Student 7 named 15 goal knowledge nodes against a preferred 90-minute maximum, and Student 11 chose 60 minutes for five goals. The transferable artifact for practice is the rubric, which lets instructional designers rank a corpus without matching materials to learning styles, encourages platform builders to treat selection and sequencing as separate optimization steps, and warns advisors to expect weaker paths when many goals meet a short time budget. The [[assessment|evaluation]] stops at rubric-level path quality, though, and the authors state that human experiments remain future work, so the link between a well-sequenced path and improved learning is assumed rather than shown, the same gap the page flags between system and learner-perceived personalization.
...
- [[personalized-learning-path-ctml-2026]] — CTML-derived material and path rubrics, and complexity proofs for material selection and sequencing in personalized learning paths (Mochocki et al. 2026)
