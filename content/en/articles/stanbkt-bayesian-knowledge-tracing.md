...
- **Researchers.** Report credible intervals whenever you compare conditions: in the perceptual-cue experiment most effects on forgetting P(F) and slipping P(S) were weak or uncertain, and the only reliable deviations were a small reduction in forgetting under incongruent color (−0.012, 95% CrI [−0.023, −0.003]) and a modest reduction in slipping under congruent spacing (−0.030, 95% CrI [−0.059, −0.001]).
- Choose the inference method from the uncertainty you need, not from accuracy: MAP at 6.83 s is a drop-in replacement for pyBKT when posterior uncertainty is not needed, while MCMC at 822 s buys full posterior inference for mastery trajectories and condition comparisons.
- Use the grouped or hierarchical BKT formulations when skills or students are sparsely observed, so estimates pool information instead of resting on unstable per-group point estimates.
- Read cue effects on guessing and slipping as changes in response behavior rather than learning gains — both cues produced sizable credible deviations in P(G) (spacing −0.058 [−0.093, −0.024]; color −0.082 [−0.121, −0.040]) while no credible effect appeared for the learning rate P(T).
- **Developers.** Ship posterior predictive checks and trace plots alongside the parameter estimates, so practitioners can judge mixing and convergence before acting on a mastery model, and treat StanBKT as a complement to [[neural-symbolic-knowledge-tracing|neural]] or neural-[[item-response-theory|IRT]] alternatives rather than a replacement for them.

## Limitations

- Runtime comparisons are not normalized across methods, since MCMC stores full parameter trajectories across four parallel chains with 1,000 warmup and 1,000 sampling iterations each.
- The scalability study is restricted to the 10 knowledge components with the most interactions in ASSISTments 2020 and to student sequences with at least 10 observations, leaving sparsely observed skills untested at scale.
