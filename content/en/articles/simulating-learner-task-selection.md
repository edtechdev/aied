...
- **Researchers.** Use the simulation-before-deployment template: fit an Additive Factors Model and Bayesian [[knowledge-tracing|knowledge tracing]] to logged interaction data, then pre-test sequencing and constraint policies across more than one domain before any student meets them, keeping the [[simulating-students|simulated learners]] parameter-grounded rather than generative.
- **Instructors.** Treat the constraint as a [[human-in-the-loop-ai|human-in-the-loop]] design decision, not a hard system rule: because strategy diversity is real and consequential, constrain behavior that is maladaptive rather than the average learner, and stay alert to whether visible constraints erode the [[agency|autonomy]] that motivated shared control.
