...
- **Designers.** Pass the mastery state explicitly and verify that it changes the output. Ablating the field left validity nearly unchanged while adaptivity dropped by up to 26.1 points, so a planner can look curriculum-aligned while effectively ignoring the learner it claims to personalize for.
- **Designers.** Keep a teacher reviewing generated sequences before they reach students. The strongest model reached only a 29.5% final pass rate in basic education, and prerequisite violations and [[hallucination-risk|hallucinated]] entities appeared often enough to be scored as separate validity failures.
- **Researchers.** Report validity, adaptivity and efficiency separately instead of one pass rate. No model exceeded 44.7% adaptivity even when validity ran above 90%, and the multiplicative final-pass product hides which dimension failed.
- **Researchers.** Add retrieval as a validity intervention, not a personalization fix. Top-3 retrieved knowledge chunks lifted validity from 39.7% to 73.8% for Qwen3-4B-2507 while efficiency fell from 25.8% to 18.4% and the final pass rate barely moved (3.2% to 2.9%).
