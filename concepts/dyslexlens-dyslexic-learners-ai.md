---
title: "DysLexLens: A Low-Resource LLM Framework for Analysing Dyslexic Learners Insights from Online Forums"
created: 2026-06-29
updated: 2026-06-29
type: concept
tags: [special-education, llm, ai-literacy, equity, student-experience, language-learning, k-12, higher-ed]
sources: [raw/papers/2606.27619.md]
confidence: high
---
# DysLexLens: A Low-Resource LLM Framework for Analysing Dyslexic Learners Insights from Online Forums

**Dana Rezazadegan, Atie Kia, Phongpadid Nandavong, Dominique Carlon, Jeremy Nguyen (2026)** — [Artificial Intelligence (cs.AI)](https://arxiv.org/abs/2606.27619)
DysLexLens is a low-resource LLM framework designed to analyze how [[special-education|dyslexic learners]] experience AI tools by mining online forum discussions. The framework employs dictionary-driven filtering to construct focused corpora from Reddit, integrates LLM-assisted knowledge graph reasoning, and generates verifiable query responses about learners' lived experiences with AI for reading, writing, and study tasks.

The research reveals that while dyslexic learners find value in AI tools for [[ai-literacy|supporting literacy]], they face significant [[accessible-learning|accessibility barriers]] including inconsistent output quality and lack of [[equity|equitable]] accommodations. This has direct implications for [[student-experience|student-AI interaction]] design and suggests that inclusive AI education must address [[language-learning|language and literacy support]] across both [[k-12|K-12]] and [[higher-ed|higher education]] settings.

By grounding analysis in real user discourse rather than controlled experiments, the work complements traditional [[llm-student-modeling-memory|student modeling approaches]] and provides an evidence base for designing AI tools that better serve neurodiverse learners.

## Related Pages
- [[ai-literacy]] — AI literacy concepts and programs
- [[equity]] — Equity in AI education
- [[student-experience]] — Student-AI interaction experiences
- [[special-education]] — Special education and AI
- [[accessible-learning]] — Accessible learning with AI
- [[language-learning]] — Language learning with AI
- [[llm-student-modeling-memory]] — LLM memory for student modeling

## Citation

Dana Rezazadegan, Atie Kia, Phongpadid Nandavong, Dominique Carlon, Jeremy Nguyen (2026). DysLexLens: A Low-Resource LLM Framework for Analysing Dyslexic Learners Insights from Online Forums. arXiv:2606.27619. Artificial Intelligence (cs.AI).
