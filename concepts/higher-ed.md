---
title: AI in Higher Education
created: 2026-05-11
updated: 2026-05-11
type: concept
tags: [higher-ed, generative-ai, llm]
sources: []
confidence: low
---

# AI in Higher Education

## Connections

## Related Pages
- [[lets-chat-chatbot-outreach-2026]] — Chatbot outreach RCTs at Georgia State
- [[critical-genai-use-predictors]] — Correlational study at Bielefeld University
- [[scheu-mobile-chatbot-journaling-motivation-2026]] — Chatbot journaling RCT at a German technical university
- [[agency-gap-ai-writing]] — AI writing support in medical/nursing education
- [[feedback-futures-genai]] — GenAI feedback governance and assessment design in universities
- [[genai-teacher-feedback-comparison]] — Four-Australian-university survey of feedback perceptions
- [[care-full-feedback-genai]] — Feedback as professional craft in university contexts
- [[genai-impact-chinese-students-hss]] — Large-scale survey examining GenAI's role in academic development of humanities and social sciences students
- [[ai-assisted-writing-research-teams]] — 6 of 8 papers in May 28 scan
- [[ethical-ai-higher-ed-game-theory]] — 3 of 8 papers in May 28 scan
- [[llm-sentiment-analysis-education-research]] — 1 of 8 papers in May 28 scan
- [[genai-minoritized-knowledges-disability]] — GenAI's role in epistemic coloniality within higher education
- [[chatgpt-critical-creative-thinking-review]] — Systematic review: ChatGPT's dual impact on critical and creative thinking in higher education (67 studies)
- [[ai-pedagogical-orientation]] — AI pedagogical orientation drives faculty AI adoption more than institutional factors
- [[international-students-conversational-ai-adaptation]] — Conversational AI for international student cross-cultural adaptation
- [[agentic-ai-ecosystems-higher-education]] — Agentic AI ecosystems framework for higher education institutions
- [[institutional-change-framework-ai]] — Six-dimension framework for adapting institutional change models in STEM to generative AI
- [[teachingcoach-chatbot-instructor-guidance]] — Higher education instructors use TeachingCoach for real-time pedagogical guidance
- [[ai-campus-wellbeing-tools]] — TigerGPT, AURA, PsychoGPT: integrated campus well-being AI framework- [[a4l-analytics-pipeline]] — A4L modular analytics pipeline for cross-domain educational AI data
- [[student-rationalization-ai-writing]] — Taxonomy of 20+ student rationalizations for AI use in academic writing
- [[culturally-aware-aied-community-learning]] — Cross-boundary community learning for undergraduate AI education
- [[instructional-guidance-genai-learning]] — GLT-based prompting framework in construction engineering education
- [[foxglove-writing-feedback-experts-llms]] — Expert vs LLM writing feedback quality across dimensions- [[physiological-signals-exam-outcomes-ml]] -- Random forest predicts exam outcomes from physiological signals as well as deep learning with better interpretability
- [[wordstream-glass-learning-analytics]] -- Revisits WordStream (2009) as a quantitative encoding for qualitative learning a...
- [[computing-students-cybersecurity-attitudes]] -- Computing students show inconsistencies between confidence in cybersecurity know...
- [[ai-psychotherapy-training-avatars]] -- AI-driven interactive patient avatars for psychotherapy training provide accessi...
- [[at-risk-students-ml-prediction]] -- Logistic regression and linear SVM achieve highest accuracy for predicting student attrition using academic, demographic, and enrollment data.
- [[tutors-gig-economy-automated-feedback]] -- Tutors perceived AI feedback more negatively than learner feedback but found it useful for self-monitoring; discrepancies between the two sources caused confusion.
- [[ai-assessment-scale-reform]] — AI Assessment Scale (AIAS) Reform
- [[prompt-problems-nl-programming-mistakes]] — Understanding Student Perceptions, Mistakes, and Debugging Approaches when Solving Natural Language Programming Tasks
- [[genai-skill-bypass-literacy]] — The GenAI Skill Bypass
- [[ai-tools-arab-english-classrooms]] — AI tools in Arab University English classrooms
- [[bloom-aligned-educational-control-llms]] — Introduces a Bloom-aligned framework for measuring 'educational control' in LLMs: the abil…
- [[ai-learning-assistants-higher-ed-large-scale]] — Presents a large-scale descriptive analysis of an AI learning assistant (Syntea) using obj…
- [[youtube-frames-chatgpt-education]] — Uses epistemic network analysis of multimodal YouTube metadata (transcripts, titles, thumb…
- [[llm-design-problems-hot-pjbl]] — Higher-ed computing (2026-07-14)
- [[q-learning-lab-rl-teaching]] — Undergraduate courses (2026-07-14)
- [[commenting-copilot-student-code-specs]] — Higher-ed contexts (2026-07-14)
- [[teaching-feedback-classification-benchmark]] — Higher-ed institutions (2026-07-14)
- [[knowledge-distillation-ai-tutor-evaluation]] — Higher-ed deployment (2026-07-14)
- [[genai-policies-higher-ed-computing]] - computing-education context
- [[learning-engagement-assistant-lea]] — agentic RAG+KC tutoring system; first real-student classroom deployment (n=8) and cross-course scalability test
- [[genai-reliance-types-scale]] — undergraduate academic-writing context
- [[mllm-scientific-visualization-literacy]] — undergraduate science-literacy assessment context
- [[assessment-team-problem-solving-computing-education]] — Assessment in Team Problem-Solving Exercises in Computing Education
- [[genai-usage-design-students-survey]] — A study of GenAI usage by Design Students: Analysis of Survey Results and Journals of AI practices at the Politecnico di Milano in 2025/2026
- [[visual-query-tracer-declarative-logic-learning]] — Evaluating a Visual Query Tracer and Builder for Learning Declarative Logic Programming
- [[embodied-string-learning-blindness-low-vision-musicians]] — Designing for What Cannot Be Seen: Supporting Embodied String Learning for Musicians with Blindness and Low-Vision
- [[genai-availability-grades-satisfaction]] — Generative AI Availability, Grades, and Student Satisfaction
- [[ai-learning-tools-engineering-education-needs]] — 206 engineering students: AI chatbots give greatest benefit as relief from compe...
- [[vibe-coding-programming-process-visualizer]] — Thonny log visualizer built via AI-assisted 'vibe coding' piloted with 160 stude...
- [[genai-architectural-design-studios]]
- [[llm-intervention-design-cs-review]]
- [[genai-mindtool-generative-learning]]
- [[multimodal-prompting-ai-literacy]]
- [[genai-runaway-object-math-higher-ed]]
- [[human-ai-collaboration-trust-expectations]]
- [[quest-ai-inquiry-preservice-teachers]]
- [[same-ai-different-pathways]]
- [[ai-generated-slides-student-perception]] — AI slide generation in university courses
- [[aicode-collaborative-feedback-system]] — Multi-LLM peer feedback system
- [[ai-assistance-discretionary-feedback]] — Higher education feedback provision context
- [[hypergamification-game-engine-lms]]
- [[structured-ai-demonstrations-engineering-mechanics]]
- [[savvy-student-attention-video-learning]]
- [[llm-facilitation-timing-online-discussions]]
