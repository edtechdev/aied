---
title: "AI in Writing Education"
created: 2026-05-09
updated: 2026-05-09
type: concept
tags: ['writing-education', 'ai-literacy', 'scaffolding']
sources: []
confidence: low
---

# AI in Writing Education

> Stub — pending source ingestion. AI applications in writing instruction, composition feedback, and argumentation development.

## Connections

## Related Pages
- [[agency-gap-ai-writing]] — Multimodal analytical writing with reactive vs proactive agents
- [[ai-assisted-writing-research-teams]] — 6 of 8 papers in May 28 scan
- [[mindcopilot-llm-co-writing]] — Formal MDP model of human-LLM co-writing with interaction-aware metrics (IJCAI 2026)
- [[vocabulary-difficulty-prediction]] — LLM fine-tuned with soft-target loss achieves r>0.91 for vocabulary difficulty p
- [[cyberscholar-genai-writing-feedback]] — Generative AI Feedback, English Writing and Teacher Rubrics: A Multiple-Case Study of CyberScholar

- [[genai-meta-analysis-programming-learning]] — Productivity-learning tension applies across programming and writing domains
- [[self-referential-l2-writing-llm-assessment]] — Extends AI in composition research to second language contexts- [[humanlike-ai-collaborative-writing]] — Humanlike AI writing tools: efficiency gains vs. surveillance feelings
- [[student-rationalization-ai-writing]] — Taxonomy of 20+ student rationalizations for AI use in academic writing

- [[foxglove-writing-feedback-experts-llms]] — FOXGLOVE: first systematic comparison of LLM vs expert writing feedback- [[aiawe-automated-writing-evaluation]] — AI-assisted writing evaluation for argumentative essays
- [[gender-bias-transfer-llm-writing]] — Contaminated Collaboration
- [[llm-educational-question-cognitive-depth]] -- LLM-generated educational questions show varying cognitive depth; models excel a...
- [[psyscore-essay-scoring-zpd-feedback]] -- PsyScore: A Psychometrically-Aware Framework for Trait-Adaptive Essay Scoring and ZPD-Scaffolded Feedback
- [[llm-reliance-types-undergrad]] — four types of LLM reliance among undergrad writers
- [[ai-tools-arab-english-classrooms]] — AI tools in Arab University English classrooms
- [[genai-reliance-types-scale]] — academic-writing pedagogy context
- [[repeated-ai-writing-feedback-semester]] — Classroom study of AI feedback integration across a semester of writing coursework.
- [[icle-plus-plus-essay-scoring]]
- [[genai-linguistic-diversity-academic-writing]]
- [[generative-refusal-ai-tools-for-thought]]
