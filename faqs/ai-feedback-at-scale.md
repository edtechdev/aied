...
- **Decide what the tool is for before you deploy it.** Copy-editing support, rubric-guided formative comments and draft diagnostics are supported uses; final grades and unmoderated judgment of quality are not.
...
- **Keep an instructor between the draft and the student.** Sentence-level curation with source attribution, as in AICoFE, preserves judgment while still saving composition time.
